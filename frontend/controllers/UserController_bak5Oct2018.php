		...
		*/
		...
		public function actionImportinvoices(){
			...
			try{
				...
				if ($client->validateCredentials()) {
					$todaydate = date("Y-m-d", strtotime("-6 month"));
		...
		public function actionDownloadpdf(){
			...
			if(isset($amazon_order_id) && $amazon_order_id !="") {
				...
				$amazonOrdersModel	= $amazonOrdersModel->checkExistingOrder($amazon_order_id, Yii::$app->user->id);
				$vat			= VatRn::getUserVat(Yii::$app->user->id);
				$content = $this->renderPartial('_invoice_pdf', ['companyModel' => $companyInfoModel,'amazonOrderModel'=>$amazonOrdersModel,'orderItems'=>$items,'vat'=>$vat,'productModel'=>$productModel]);
		...
		public function actionDownloadcreditmemopdf(){
			...
			if(isset($amazon_order_id) && $amazon_order_id !="") {
				...
				//$invoiceNo = $amazonCreditModel->invoice_number;
				$vat			= VatRn::getUserVat(Yii::$app->user->id);
				$content		= $this->renderPartial('_invoice_creditmemopdf', ['order_id'=>$amazon_order_id,'companyModel' => $companyInfoModel,'amazonOrderModel'=>$orderDetails,'orderItems'=>$items,'refundAmount'=>$refundAmount,'orderItems'=>$items,'amazonCreditModel'=>$amazonCreditModel,'vat'=>$vat,'productModel'=>$productModel]);
				$mpdf = new Mpdf(['mode' => 'utf-8', 'format' => 'A4-P', 'default_font_size' => 9,'default_font' => 'Tahoma,sans-serif']);
				$mpdf->WriteHTML($content);
				$mpdf->Output();
				$this->insertLog("Invoice pdf created");
