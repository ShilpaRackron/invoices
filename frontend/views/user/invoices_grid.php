...
<p>
    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            ['class' => 'yii\grid\ActionColumn','template'=>'{view}<br/><br/>{sendpdf}<br/><br/>{setvat}', 'buttons' => [
...
[ 'title' => Yii::t('app', 'View'), 'class'=>'btn btn-primary btn-xs', ]) ;
					...
					return 	Html::a('<span class="fa fa-file-pdf-o" title="Send PDF"></span>PDF','#', ['class'=>'btn btn-primary', 'title' => Yii::t('yii', 'Send PDF'), 'onclick'=>" $.ajax({
					type     :'POST',
					cache    : false,
					data:{is_ajax: true },
					url  : '".$url."',
					...
					'setvat' => function ($url, $model) {
						...
						$setVatText = (trim($checkVatExist) !="")?"EditVAT":"SetVAT";
                        return  Html::a('<span class="fa fa-pdf" title="'.$setVatText.'"></span>'.$setVatText, '#', 
[ 'title' => Yii::t('app', $setVatText), 'class'=>'btn btn-xs setvat',  "url"=>$url ]) ;
                    },
                ],
